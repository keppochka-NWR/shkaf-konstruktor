...
export function RoomPlan({project,active,onSelect,onRoom,update}:Props){
  ...
  </svg><div className={"plan-help"+(problem?" invalid":"")} role={problem?"alert":undefined}>{problem||warnings[0]?.message||"Тяните корпуса и проёмы · Толстая линия — фасад · Пунктир — верхний модуль"}</div></div>;
}
