...
export function movePart(p:Project,mid:string,sid:string,pid:string,delta:number){
  ...
  else if(pid.includes(':rod')||pid.includes(':pantograph:')){const current=parts(m).find(p=>p.id===sid+':rod'||p.id===sid+':pantograph:rod')!;s.rodAt=(current.position[1]+delta-b.bottom)/(b.top-b.bottom);}
  return n;
}
