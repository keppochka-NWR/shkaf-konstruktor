...
type Props={room:Room;selected?:string;onSelect:(id:string)=>void;onChange:(room:Room)=>boolean;part?:'walls'|'openings'|'all'};
const names={back:'Задняя стена',left:'Левая стена',right:'Правая стена',front:'Передняя стена'};
export function RoomEditor({room,onChange,selected,onSelect,part='all'}:Props){
  ...
  return <div className="room-openings">{part!=='openings'&&<><h3>Стены</h3><label className="hardware-field">Толщина стен<input type="number" aria-label="Толщина стен" key={room.walls?.thickness??120} defaultValue={room.walls?.thickness??120} onBlur={e=>{const v=Number(e.target.value);if(!onChange({...room,walls:{...room.walls,thickness:v}}))e.target.value=String(room.walls?.thickness??120);}} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur();}}/></label>
    ...
    {part!=='walls'&&<><h3>Окна и двери</h3><div className="room-add"><button onClick={()=>add('window')}><Plus size={14}/> Окно</button><button onClick={()=>add('door')}><Plus size={14}/> Дверь</button></div>
