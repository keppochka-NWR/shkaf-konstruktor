...
export function RoomFixtures({room,selected,onSelect,onChange,roomName,onImport}:Props){
  ...
  return <div className="room-openings"><h3>Объекты на стенах</h3>
    ...
    <p className="field-note">Config.xml лежит рядом со скриптом «Замер помещения» в папке Базиса и хранит последний заполненный замер. «Сохранить для Базиса» делает такой же файл: положите его в папку скрипта и запустите скрипт.</p>
