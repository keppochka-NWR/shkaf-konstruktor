...
export const LibraryPreview=memo(function LibraryPreview({module,group}:{module:Module;group?:PlacedModule[]}){
 const drawing=useMemo(()=>{
  ...
  for(const a of bodies){
   const ps=bodies.length>8?[{size:[a.module.width,a.module.height,a.module.depth],position:[a.module.width/2,a.module.height/2,a.module.depth/2],role:'body',material:'board',id:'body'}]:parts(a.module).filter(p=>p.role!=='door'&&p.id!=='back'&&p.role!=='hinge'&&p.role!=='handle'&&!p.id.includes(':slide:'));
   for(const p of ps){
