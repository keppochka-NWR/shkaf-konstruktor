...
test('removing a drawer preserves the absolute height of remaining drawers',()=>{const p=newProject(),a=p.modules[0],s=a.module.sections[0];const before=parts(a.module).find(p=>p.id===s.id+':drawer:1:left')!;const n=removePart(p,a.id,s.id,s.id+':drawer:0:bottom');assert.deepEqual(projectErrors(n),[]);assert.equal(n.modules[0].module.sections[0].drawers,1);assert.equal(parts(n.modules[0].module).find(p=>p.id===s.id+':drawer:0:left')!.position[1],before.position[1]);assert.equal(s.drawers,2);const empty=removePart(n,a.id,s.id,s.id+':drawer:0:bottom');assert.ok(!parts(empty.modules[0].module).some(p=>p.id.includes(':drawer-cap')||p.id.includes(':filler:')));});
...
test('estimate uses actual sheets and never substitutes unknown hardware prices',()=>{const p=newProject();p.modules[0].module.sections[0].drawerConfigs=[{slide:'gtv0fpo',length:550,height:140},{slide:'gtv0fpo',length:550,height:140}];const e=estimate(p);assert.equal(e.lines.filter(l=>l.id.startsWith('sheet:')).reduce((s,l)=>s+l.quantity,0),nest(p).length);assert.equal(e.retail,null);assert.ok(e.missing.some(l=>l.id==='slide:gtv0fpo:550'));p.calculation={markup:2.2,overrides:{'slide:gtv0fpo:550':800}};const complete=estimate(p);assert.equal(complete.missing.length,0);assert.equal(complete.retail,Math.round(complete.knownCost*2.2/100)*100);assert.equal(complete.lines.find(l=>l.id==='slide:gtv0fpo:550')!.quantity,2);assert.deepEqual(parseProject(p),p);});
...
test('review package contains consistent details and internal review warnings',()=>{
  ...
  p.modules[0].module.sections[0].drawerConfigs=[{slide:'gtv0fpo',length:550,height:140},{slide:'gtv0fpo',length:550,height:140}];
